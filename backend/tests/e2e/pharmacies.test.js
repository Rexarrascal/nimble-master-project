...
test("get for all pharmacies", async () => {
  ...
  await request.post("/pharmacies").send(pharmacyToCreate).expect(200);

  const responseForGet = await request.get(`/pharmacies`).expect(200);

  expect(JSON.stringify(responseForGet)).toContain(`${pharmacyToCreate.name}`);
});
...
test("patch a pharmacy", async () => {
  ...
  expect(flatted.stringify(responseForPatch)).toContain(
    `The name of the pharmacy changed to ${pharmacyNewName.name}`
  );
});
