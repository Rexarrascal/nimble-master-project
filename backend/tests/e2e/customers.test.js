...
test("get for all customers", async () => {
  ...
  await request.post("/customers").send(customerToCreate).expect(200);

  await request.post("/customers").send(customerToCreate2).expect(200);

  const responseForGet = await request.get(`/customers`).expect(200);
...
test("patch a customer", async () => {
  ...
  expect(flatted.stringify(responseForPatch)).toContain(
    `The name of the customer changed to ${customerNewName.name}`
  );
});
